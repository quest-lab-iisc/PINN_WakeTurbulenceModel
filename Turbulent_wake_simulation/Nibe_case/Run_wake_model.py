...
from Nibe_wake_model import PdeModel, get_ibc_and_inner_data
...
u_ref=8.5      #reference velocity
z_ref=45.      #reference height
TI_ref=0.1     #reference turbulence intensity
Radius=20.     #radius of turbine
C_T=0.82       #thrust coefficient
...
x_lim = max(x_train[:,0:1])
y_lim = max(x_train[:,1:2])
z_lim = max(x_train[:,2:3])

input1 = layers.Input(shape=(1,), name='x_input')
input_x_rescale = layers.Rescaling(scale=100/x_lim, offset=0)(input1)
input2 = layers.Input(shape=(1,), name='y_input')
input_y_rescale = layers.Rescaling(scale=100/y_lim, offset=0)(input2)
input3 = layers.Input(shape=(1,), name='z_input')
input_z_rescale = layers.Rescaling(scale=100/z_lim, offset=0)(input3)
x = layers.Concatenate()([input_x_rescale, input_y_rescale, input_z_rescale])
x = layers.Dense(units=600, activation='swish', kernel_initializer=initializer)(x)
x = layers.Dense(units=600, activation='swish', kernel_initializer=initializer)(x)
x = layers.Dense(units=600, activation='swish', kernel_initializer=initializer)(x)
x = layers.Dense(units=600, activation='swish', kernel_initializer=initializer)(x)

u_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)
v_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)
w_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)

p_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)

k_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)

e_layer = layers.Dense(units=500, activation='swish', kernel_initializer=initializer)(x)
...
ow = layers.Dense(units=1, use_bias=False, kernel_initializer=initializer, name='output_w')(w_layer)
...
model = keras.Model([input1, input2, input3], [ou, ov, ow, op, ok, oeps])
...
log_dir = 'log_output/'
history = cm.run(epochs=25000, proj_name="Actuator_disc_3D_nibe", log_dir=log_dir,
