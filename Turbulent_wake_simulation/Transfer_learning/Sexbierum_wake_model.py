...
def get_ibc_and_inner_data(nu, u_ref, z_ref, TI_ref, Radius):
    ...
    lb_refined = [-6.*Radius + 0 * Radius, -2*Radius, -2*Radius+z_ref]
    ub_refined = [6.*Radius + 0 * Radius, 2*Radius, 2*Radius+z_ref]

    refined_loc = HammersleySampling([lb_refined, ub_refined], sampling_type='creation', number_of_samples=5000).sample_points()
    R_refined = np.linalg.norm((refined_loc[:,1:2],refined_loc[:,2:3]-z_ref),axis=0)
    indices_refined = np.where((R_refined > 2*Radius) | ((R_refined<=Radius)&(np.abs(refined_loc[:,0:1] - 0 * Radius)<=0.5*Radius)))
    refined_loc[indices_refined[0]]=np.nan
    refined_loc = (refined_loc[np.logical_not(np.isnan(refined_loc))]).reshape(-1,3)  

    lb_disk = [-0.5*Radius + 0 * Radius, -Radius, -Radius+z_ref]
    ub_disk = [0.5*Radius + 0 * Radius, Radius, Radius+z_ref]

    x_disk_loc = HammersleySampling([lb_disk, ub_disk], sampling_type='creation', number_of_samples=5000).sample_points()
    ...
    train_domain = HammersleySampling([lb, ub], sampling_type='creation', number_of_samples=50000).sample_points()
    RR = np.linalg.norm((train_domain[:,1:2],train_domain[:,2:3]-z_ref),axis=0)
    indices_domain = np.where((RR<= 2*Radius) & (np.abs(train_domain[:,0:1] - 0 * Radius)<6*Radius))
...
class PdeModel:
    ...
    def act_disc_pde_residual(self, x_disk, y_disk, z_disk, training=True):
        ...
        # Momentum equation calculation
        fx = (u * ux + v * uy + w * uz + px) - (fx_xx + fx_yy + fx_zz) + (self.C_T*0.5*((self.u_ref)**2)*tf.exp(-0.5*(x_disk)**2/(2.5*2.5))/(2.5*tf.sqrt(2*np.pi)))
        fy = (u * vx + v * vy + w * vz + py) - (fy_xx + fy_yy + fy_zz)
        fz = (u * wx + v * wy + w * wz + pz) - (fz_xx + fz_yy + fz_zz)
    ...
    def train_step(self, xb_inlet, yb_inlet, zb_inlet, ub_inlet, vb_inlet, wb_inlet, kb_inlet, epsb_inlet,
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            residual_loss = 10*(fx_loss  + fy_loss + fz_loss) +  100*div_loss  + 1*(keq_loss) + (epseq_loss) # + Pey_loss
            residual_act_loss = 10*(fx_act_loss  + fy_act_loss + fz_act_loss) +  100*div_act_loss  + 1*(keq_act_loss) + (epseq_act_loss) # + Pey_loss
            #total loss
            loss = 100*std_loss + residual_loss + residual_act_loss
